...
- 데이터 정합성과 transaction 관리를 위해 RDB를 사용하며, 구현 DB는 MySQL입니다.
- 투표 원장과 집계 데이터는 MySQL transaction과 제약 조건을 기준으로 관리합니다.
...
- 클라이언트 측에서는 요청마다 idempotency key를 사용해 버튼 연타나 네트워크 재시도로
  같은 투표 요청이 여러 번 전송되는 상황을 줄일 수 있습니다.
...
- 클라이언트 idempotency key는 서버의 DB 방어선과 별개로 사용자 경험 측면의 중복 요청을
  줄이는 보조 수단입니다. 최종 중복 판단은 서버의 `voterId` 기준 검증과 DB 제약이 담당합니다.
